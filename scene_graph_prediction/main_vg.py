...
from scene_graph_prediction.scene_graph_helpers_vg.dataset.vg_dataset import VGDataset
from scene_graph_prediction.scene_graph_helpers_vg.model.scene_graph_prediction_model_vg import SGPNModelWrapperVG
from pytorch_lightning.callbacks import ModelCheckpoint
from torchinfo import summary


def config_loader(config_path: str):
    config_path = Path('scene_graph_prediction/scene_graph_helpers_vg/configs') / config_path
...
def main():
    ...
    logger = pl.loggers.TensorBoardLogger('scene_graph_prediction/scene_graph_helpers_vg/logs', name=name, version=0)
    checkpoint_path = find_checkpoint_path(logger.log_dir)
    token_counter = Counter()
    if mode == 'train':
        train_dataset = VGDataset(config, 'train')
        val_dataset = VGDataset(config, 'val')
        train_loader = DataLoader(train_dataset, batch_size=config['BATCH_SIZE'], shuffle=True, num_workers=config['NUM_WORKERS'], pin_memory=True,
                                  collate_fn=train_dataset.collate_fn)
        val_loader = DataLoader(val_dataset, batch_size=config['BATCH_SIZE'], shuffle=False, num_workers=config['NUM_WORKERS'], pin_memory=True,
                                collate_fn=val_dataset.collate_fn)

        model = SGPNModelWrapperVG(config, ind_to_classes=train_dataset.ind_to_classes, ind_to_predicates=train_dataset.ind_to_predicates)
        ...
        # For pix2sg scenario, count the frequency of each token in the training set, and save it as json
        ...
        # with open('scene_graph_prediction/scene_graph_helpers_vg/dataset/pix2sg_pred_frequencies.json', 'w') as f:
        #     json.dump({k:v for k,v in sorted(token_counter.items())}, f)
        # with open('scene_graph_prediction/scene_graph_helpers_vg/dataset/pix2sg_token_frequencies.json', 'w') as f:
        #     json.dump({k:v for k,v in sorted(token_counter.items())}, f)
        ...
        train_dataset = VGDataset(config, 'train')
        eval_dataset = VGDataset(config, 'val')
        ...
        checkpoint_path = 'scene_graph_prediction/scene_graph_helpers_vg//logs/pix2sg_b5/version_0/checkpoints/epoch=129.ckpt'
        # checkpoint_path = 'scene_graph_prediction/scene_graph_helpers_vg//logs/pix2sg_b7/version_0/checkpoints/epoch=129.ckpt'
        # checkpoint_path = 'scene_graph_prediction/scene_graph_helpers_vg/logs/pix2sg_eva_base_freeze8/version_0/checkpoints/epoch=129.ckpt'
        # checkpoint_path = 'scene_graph_prediction/scene_graph_helpers_vg/logs/pix2sg_eva_large_freeze16/version_0/checkpoints/epoch=129.ckpt'
        model = SGPNModelWrapperVG.load_from_checkpoint(checkpoint_path=checkpoint_path, config=config, ind_to_classes=train_dataset.ind_to_classes,

...
from scene_graph_prediction.scene_graph_helpers_psg.model.pix2sg_model.detr_configs import N_PREDS, N_ENTITIES, ENTITY_START, PRED_START, INSTANCE_START, \
    ...
    def forward(self, image_src, image_mask, image_pos_embed, seq, vocab_embed, max_num_rels=None):
        ...
        if self.training:
            ...
            for i in range(ITERATON_STEPS):
                ...
                # # TODO if you want to respect the prediction of END_TOKEN and stop the prediction, activate this
                # # check if EOS_TOKEN (END_Token) is the most probable token in out. Using argmax is one option. this only makes sense for batchsize=1
                # if out.shape[0] == 1 and out.argmax() == END_TOKEN:
                #     # make sure values are still dividable by TOKENS_PER_REL. Thhis will mean removing the last incomplete relation. Which might be 1 to 4 tokens
                #     correct_length = (i // TOKENS_PER_REL) * TOKENS_PER_REL + 1
                #     seq = seq[:, :correct_length]
                #     values = values[:correct_length-1]
                #     break

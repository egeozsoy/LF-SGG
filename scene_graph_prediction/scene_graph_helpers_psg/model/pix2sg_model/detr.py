...
from scene_graph_prediction.scene_graph_helpers_psg.model.pix2sg_model.backbone import build_backbone
from scene_graph_prediction.scene_graph_helpers_psg.model.pix2sg_model.detr_configs import NO_KNOWN_TOKEN, MAX_NUM_RELS, RANDOM_MAX_INSTANCE_ID, \
    INSTANCE_START, START_TOKEN, NOISE_TOKEN, END_TOKEN, ASSUMED_MAX_INSTANCE_ID, PRED_START, ENTITY_START, N_PREDS, N_ENTITIES
from scene_graph_prediction.scene_graph_helpers_psg.model.pix2sg_model.transformer import build_transformer
from scene_graph_prediction.scene_graph_helpers_psg.model.pix2sg_model.util.misc import (NestedTensor, nested_tensor_from_tensor_list)
...
def preprocess_input(batch, device, for_train=True):
    ...
    if for_train:
        ...
        for elem in batch[1]:
            ...
            random_sub_ids = torch.randint(N_PREDS, N_PREDS + N_ENTITIES + 1, (MAX_NUM_RELS - len(rel_label),)).long().cpu().unsqueeze(1)
            random_obj_ids = torch.randint(N_PREDS, N_PREDS + N_ENTITIES + 1, (MAX_NUM_RELS - len(rel_label),)).long().cpu().unsqueeze(1)
            random_pred_ids = torch.randint(0, N_PREDS + 1, (MAX_NUM_RELS - len(rel_label),)).long().cpu().unsqueeze(1)
...
def build(config):
    ...
    with open('scene_graph_prediction/scene_graph_helpers_psg/dataset/pix2sg_pred_frequencies.json') as f:

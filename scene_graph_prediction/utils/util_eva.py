...
from scene_graph_prediction.utils.plot_confusion_matrix import plot_confusion_matrix
...
def get_mean_metric(confusion: np.array, VALID_CLASS_IDS: list, CLASS_LABELS: list, return_individuals=False):
    ious = dict()
    precisions = dict()
    recalls = dict()
        ...
        def write_metric(name, values):
            ...
            for i in range(len(VALID_CLASS_IDS)):
                ...
                if isinstance(values[label_name], tuple):
                    ...
                    f.write('{0:<14s}({1:<2d}): {2:>5.3f}   ({3:>6d}/{4:<6d})\n'.format(label_name, label_id, values[label_name][0], values[label_name][1],
                                                                                        values[label_name][2]))
                    sum += values[label_name][0]
                    counter += 1
                else:
...
class EvalSceneGraph():
    ...
    def get_individual_rel_metrics(self):
        return get_mean_metric(self.eva_r_cls.c_mat, [], self.eva_r_cls.class_names, return_individuals=True)

    def get_rel_reporting_by_class(self):
        ious, precisions, recalls = self.get_individual_rel_metrics()
        keys = ious.keys()
        table = PrettyTable(['REL', 'IOU', 'PREC', 'RECALL', 'TP'])
        for key in sorted(keys):
            iou_values = ious[key]
            prec_values = precisions[key]
            recall_values = recalls[key]
            if isinstance(iou_values, tuple):
                iou, tp, _ = iou_values
            else:
                iou = 0.0
                tp = 0.
            if isinstance(prec_values, tuple):
                precision = prec_values[0]
            else:
                precision = 0.
            if isinstance(recall_values, tuple):
                recall = recall_values[0]
            else:
                recall = 0.
            table.add_row([key, f'{iou:.3f}', f'{precision:.3f}', f'{recall:.3f}', f'{tp:.3f}'])
